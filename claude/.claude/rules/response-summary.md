# Response Summary

At the end of every response, include a brief spoken summary for text-to-speech in this exact format:

```
[SPOKEN_SUMMARY]: <1-2 sentence summary of what was accomplished>
```

Guidelines for the summary:

- Keep it under 150 characters
- Use natural spoken language (avoid technical jargon, file paths, or code)
- Focus on what was done, not how
- Example: "I fixed the login bug and added input validation."
- Example: "I found 3 files that handle authentication."
